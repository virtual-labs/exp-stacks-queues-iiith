...
function change_interval() {
  ...
  if (document.getElementById("interval").value != 100) {
    queue_artefact.interval = setInterval(
      check,
      3400 - document.getElementById("interval").value
    );
    document.getElementById("pause-queuedemo").style.backgroundColor =
      "#288ec8";
  } else
...
function drawQueueStructure(ctx) {
  ...
  if (headhere >= -1 && tailhere >= -1) {
    ...
    var txtaWidth = queueA_canvas.measureText(txta).width;
    var txtbWidth = queueB_canvas.measureText(txtb).width;
    ...
    ctx.fillStyle = "#a4c652";
    ctx.fillRect(
      queue_artefact.rectStartx +
        headhere * queue_artefact.rectWidth +
        headhere * queue_artefact.arrayDist,
      queue_artefact.rectStarty,
      queue_artefact.rectWidth,
      queue_artefact.rectHeight
    );
    for (var i = headhere; i <= tailhere; i++) {
      ctx.fillStyle = "#288ec8";
      ctx.fillRect(
        queue_artefact.rectStartx +
          i * queue_artefact.rectWidth +
          i * queue_artefact.arrayDist,
        queue_artefact.rectStarty,
        queue_artefact.rectWidth,
        queue_artefact.rectHeight
      );
    ...
    ctx.fillRect(
      queue_artefact.rectStartx +
        tailhere * queue_artefact.rectWidth +
        tailhere * queue_artefact.arrayDist,
      queue_artefact.rectStarty,
      queue_artefact.rectWidth,
      queue_artefact.rectHeight
    );
...
function writeNumbers(ctx) {
  ...
  if (headhere > -1 && valueshere.length > 0 && tailhere > -1) {
    for (var i = headhere; i <= tailhere; i++) {
      ...
      txtX =
        ...
        i * queue_artefact.rectWidth +
        i * queue_artefact.arrayDist +
        (queue_artefact.rectWidth - txtWidth) / 2;
      txtY =
        queue_artefact.rectStarty +
        queue_artefact.rectHeight -
        (queue_artefact.rectHeight - queue_artefact.txtSize) / 2;
  ...
  if (headhere > -1) {
    var relativeArrowPos =
      headhere * queue_artefact.rectWidth + headhere * queue_artefact.arrayDist;

    ctx.beginPath();

    ctx.moveTo(
      queue_artefact.hArrowStartx + relativeArrowPos,
      queue_artefact.hArrowStarty
    );
    ctx.lineTo(
      queue_artefact.hArrowEndx + relativeArrowPos,
      queue_artefact.hArrowEndy
    );
    ctx.strokeStyle = "#a4c652";
    ctx.lineWidth = 1.5;
    ctx.stroke();
    ...
    ctx.lineTo(
      queue_artefact.hArrowStartx +
        relativeArrowPos -
        queue_artefact.hArrowHeadOffsetx,
      queue_artefact.hArrowStarty - queue_artefact.hArrowHeadOffsety
    );
    ctx.strokeStyle = "#a4c652";
    ctx.stroke();
    ...
    ctx.fillText(
      htxt,
      queue_artefact.hArrowEndx + relativeArrowPos - htxtWidth / 2,
      queue_artefact.hArrowEndy - queue_artefact.hArrowHeady
    );
  ...
  if (tailhere > -1) {
    var relativeArrowPos =
      tailhere * queue_artefact.rectWidth + tailhere * queue_artefact.arrayDist;

    ctx.beginPath();

    ctx.moveTo(
      queue_artefact.tArrowStartx + relativeArrowPos,
      queue_artefact.tArrowStarty
    );
    ctx.lineTo(
      queue_artefact.tArrowEndx + relativeArrowPos,
      queue_artefact.tArrowEndy
    );
    ...
    ctx.lineTo(
      queue_artefact.tArrowStartx +
        relativeArrowPos -
        queue_artefact.tArrowHeadOffsetx,
      queue_artefact.tArrowStarty + queue_artefact.tArrowHeadOffsety
    );
    ctx.strokeStyle = "#7bde3d";
    ctx.stroke();
    ...
    ctx.fillText(
      ttxt,
      queue_artefact.tArrowEndx + relativeArrowPos - ttxtWidth / 2,
      queue_artefact.tArrowEndy + queue_artefact.tArrowTaily
    );
...
function enqueuetoa() {
  ...
  if (value === "" || value === null) return;
    ...
    document.getElementById("ins").innerHTML =
      "The letter Dequeued from queue A (i.e " +
      value +
      " ) is Enqueued to <b>Queue A</b> as it is not the required one";
    writeNumbers(queueA_canvas);
    writeNumbers(queueB_canvas);
  }
}
function check() {
  if (queue_artefact.values.length <= 0) {
    ...
    queue_artefact.correct[queue_artefact.count] === queue_artefact.values[0]
  ) {
    document.getElementById("ins").innerHTML =
      "The letter pointed by the Front pointer (i.e " +
      queue_artefact.values[0] +
      " ) is the required one so it is Dequeued from <b>Queue A</b> and Enqueued to <b>Queue B</b>";
    ...
    if (queue_artefact.checker === 0) {
      ...
      document.getElementById("ins").innerHTML =
        "The letter pointed by the Front pointer (i.e " +
        queue_artefact.values[0] +
        " ) is not the required one so it is Dequeued from <b>Queue A</b>";
      dequeuea();
    }
  }
}
...
function handlers() {
  ...
  document.getElementById("startbutton-queuedemo").onclick = function () {
    onstart();
  };
  document.getElementById("reset-queuedemo").onclick = function () {
    onreset();
  };
  document.getElementById("pause-queuedemo").onclick = function () {
    onpause();
  };
}
