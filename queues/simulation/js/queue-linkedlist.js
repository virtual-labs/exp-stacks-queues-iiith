$("#enqueue-button-qll").on("click", function () {
  insertAtHead();
});
$("#dequeue-button-qll").on("click", function () {
  popper();
});
...
function make_image() {
  ...
  base_image.src =
    ARTEFACT_URL +
    "/stacks-and-queues/img/slldemosearch-rectangle-19-copy-13@2x.png";
  base_image.width = 20;
...
function insertAtHead() {
  ...
  if (value == "" || null) {
    ...
    document.getElementById("ins").innerHTML = value + " is enqueued";
    if (numbers.length >= 7) {
      document.getElementById("ins").innerHTML = "Queue is full";
      setTimeout(function () {
...
function popper() {
  ...
  value = document.getElementById("HeadtoBeInserted").value;
  if (clearint == 1) {
    document.getElementById("ins").innerHTML = "Queue is cleared";
    clearint = 0;
    color_stopperb = setInterval(searcher, 500, value);
    busy = 0;
    return;
  } else if (numbers.length == 0) {
    document.getElementById("ins").innerHTML = "Queue is empty";
    busy = 0;
    return;
  } else if (value === "" || value === null) {
    // If no value is entered, just dequeue the front
    var dequeuedValue = numbers.shift();
    document.getElementById("ins").innerHTML =
      dequeuedValue + " is dequeued (front element)";
    renderer();
    busy = 0;
    return;
  } else if (numbers[0] == value) {
    // If the value is at the front, dequeue it
    numbers.shift();
    document.getElementById("ins").innerHTML =
...
//    };
//
//    obj.queuelinkedlist = queuelinkedlist;
//    return obj;
//
//};
