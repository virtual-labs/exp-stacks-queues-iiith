...
function insertAtHead() {
  ...
  if (value == '' || null) {
    document.getElementById('ins').innerHTML =
      'Enter the element which is to be Pushed into the stack'
    busy = 0
    return
  } else {
    document.getElementById('ins').innerHTML =
...
function popper() {
  if (busy == 1) return // Prevent multiple operations at the same time
  else busy = 1
  ...
  // Handle normal pop operation
  const poppedValue = numbers.shift() // Remove the first element
  document.getElementById(
    'ins'
  ).innerHTML = `${poppedValue} is popped from the stack`
