...
function drawStackStructure(ctx) {
  ...
  ctx.rect(
    ...
    stack_exercise_artefact.rectWidth,
    stack_exercise_artefact.rectHeight * stack_exercise_artefact.rectNo +
      stack_exercise_artefact.lineNo * stack_exercise_artefact.lineDist
  );
  ctx.closePath();
}
function writeNumbers(ctx) {
  ...
  for (var i = 0; i < values.length; i++) {
    if (ctx === stackA_canvas) ctx.fillStyle = "#a4c652";
    else ctx.fillStyle = "#288ec8";
    ctx.fillRect(
      stack_exercise_artefact.rectStartx,
      stack_exercise_artefact.rectStarty +
        stack_exercise_artefact.lineDist * stack_exercise_artefact.lineNo +
        stack_exercise_artefact.rectNo * stack_exercise_artefact.rectHeight -
        (i + 1) * stack_exercise_artefact.rectHeight -
        i * stack_exercise_artefact.lineDist,
      stack_exercise_artefact.rectWidth,
      stack_exercise_artefact.rectHeight
    );
  }
  ctx.closePath();
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    txtX =
      stack_exercise_artefact.rectStartx +
      (stack_exercise_artefact.rectWidth - txtWidth) / 2;
    txtY =
      stack_exercise_artefact.rectStarty +
      stack_exercise_artefact.lineDist * stack_exercise_artefact.lineNo +
      stack_exercise_artefact.rectNo * stack_exercise_artefact.rectHeight -
      i * stack_exercise_artefact.lineDist -
      (i + 1) * stack_exercise_artefact.rectHeight +
      stack_exercise_artefact.rectHeight / 2 +
      8;
...
function pushtoa() {
  ...
  if (stack_exercise_artefact.valuesB.length > 0) {
    ...
    document.getElementById("ins").innerHTML =
      value + " is popped from stackB and pushed to stackA";
...
function pushtob() {
  ...
  if (stack_exercise_artefact.valuesA.length > 0) {
    ...
    document.getElementById("ins").innerHTML =
      value + " is popped from stackA and pushed to stackB";
...
function popa() {
  ...
  if (stack_exercise_artefact.valuesA.length > 0) {
    ...
    document.getElementById("ins").innerHTML =
      "Your output: " + stack_exercise_artefact.output;
    writeNumbers(stackA_canvas);
  }
}
function popb() {
...
function generateQuestion() {
  ...
  for (var i = 1; i < 8; i++)
    if (
      stack_exercise_artefact.valuesA[i] <
      stack_exercise_artefact.valuesA[i - 1]
    )
      flag = 1;
  if (flag === 0)
    stack_exercise_artefact.valuesA[4] = stack_exercise_artefact.valuesA[3] / 2;
...
function onsubmit() {
  ...
  if (stack_exercise_artefact.output.length < 8) {
    str = "Incorrect <br> Your Output: ";
    for (var j = 0; j < stack_exercise_artefact.output.length - 1; j++) {
      str = str + stack_exercise_artefact.output[j] + ",";
    }
    if (stack_exercise_artefact.output.length != 0) {
      str =
        str +
        stack_exercise_artefact.output[
          stack_exercise_artefact.output.length - 1
        ];
    }
    ...
    for (var i = 1; i < stack_exercise_artefact.output.length; i++) {
      if (
        stack_exercise_artefact.output[i] <
        stack_exercise_artefact.output[i - 1]
      ) {
        str = "Incorrect <br> Your Output: ";
        for (var j = 0; j < stack_exercise_artefact.output.length - 1; j++) {
          str = str + stack_exercise_artefact.output[j] + ",";
...
function handlers() {
  document.getElementById("push-button-to-B").onclick = function () {
    pushtob();
  };
  document.getElementById("push-button-to-A").onclick = function () {
    pushtoa();
  };
  document.getElementById("pop-button-A").onclick = function () {
    popa();
  };
  document.getElementById("pop-button-B").onclick = function () {
    popb();
  };
  document.getElementById("generate-question-stackex").onclick = function () {
    generateQuestion();
  };
  document.getElementById("checkButton-stckex").onclick = function () {
    onsubmit();
  };
  document.getElementById("reset-stckex").onclick = function () {
    reset();
  };
}
